...
private:
	...
	void createSwapChain() {
		SwapChainSupportDetails swapChainSupport = quererySwapChainSupport(physicalDevice);
		...
		QueueFamilyIndices indices = findQueueFamilies(physicalDevice);
		uint32_t queueFamilIndices[] = { indices.graphicsFamily.value(), indices.presentFamily.value() };

		if (indices.graphicsFamily != indices.presentFamily) {
			createInfo.imageSharingMode = VK_SHARING_MODE_CONCURRENT;
			createInfo.queueFamilyIndexCount = 2;
			createInfo.pQueueFamilyIndices = queueFamilIndices;
		} else {
	...
	void createGraphicsPipeline() {
		...
		colourBlendAttatchment.blendEnable = VK_FALSE;
		colourBlendAttatchment.srcColorBlendFactor = VK_BLEND_FACTOR_ONE;
		colourBlendAttatchment.dstColorBlendFactor = VK_BLEND_FACTOR_ZERO;
	...
	SwapChainSupportDetails quererySwapChainSupport(VkPhysicalDevice device) {
	...
	bool isDeviceSuitable(VkPhysicalDevice device) {
		...
		if (extensionsSupported) {
			SwapChainSupportDetails swapChainSupport = quererySwapChainSupport(device);
